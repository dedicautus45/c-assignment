...
13.	Modify the above program by adding one more functionality to read output.txt and display the number of 
students who have obtained Grade A.
----------------------------------------------------------------------------------------------------------------------------------*/
...
void getGradecountA()
{
	...
	for(i=0; ;i++)
	{
		if(fscanf(output,"%s %s %d %c",&Student[i].name,&Student[i].course,&Student[i].marks,&Student[i].grade) == EOF)
			break;
